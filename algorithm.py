...
def calculateOverallEntropy(dataBelow, dataAbove):
    ...
    return pDataBelow * calculate(dataBelow) + pDataAbove * calculate(dataAbove)
...
def buildTree(dataFrame, currentDepth = 0, minSampleSize = 2, maxDepth = 1000, randomAttributes = None, randomSplits = None):
    if currentDepth == 0:
    ...
    if checkPurity(data) or len(data) < minSampleSize or currentDepth == maxDepth:
        ...
        currentDepth += 1
        potentialSplits = getSplits(data, randomAttributes)
        ...
        if len(dataBelow) == 0 or len(dataAbove) == 0:
            ...
            yesAnswer = buildTree(dataBelow, currentDepth, minSampleSize, maxDepth, randomAttributes, randomSplits)
            noAnswer = buildTree(dataAbove, currentDepth, minSampleSize, maxDepth, randomAttributes, randomSplits)
...
def predict(dataFrame, decisionTree):
    predictions = dataFrame.apply(classifySample, axis = 1, args = (decisionTree,))
    return predictions

def getSplits(data, randomAttributes):
    potentialSplits = {}
